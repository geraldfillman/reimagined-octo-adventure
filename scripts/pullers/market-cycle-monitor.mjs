/**
 * market-cycle-monitor.mjs - writes My_Data market cycle status notes.
 *
 * Raw source artifacts stay in My_Data. Generated status notes live in
 * My_Data reports and link back to source pull notes only.
 */
...
const GENERATED_BY = 'My_Data Market Cycle Monitor';
...
export function resolveCycleStatusRoot(config, reviewRoot = getReviewVaultRoot()) {
  const configuredDir = typeof config === 'string'
    ? config
    : config?.cycle_status_dir;
  const routedDir = routeCycleStatusDir(configuredDir);
  const resolvedRoot = resolve(reviewRoot);
  const resolvedTarget = resolve(resolvedRoot, routedDir);
...
export function makeCycleStatusNote(input) {
  ...
  const frontmatter = buildCycleFrontmatter({
    ...
    tags: ['world-machine-review', 'market-cycle-monitor'],
  });
...
export async function pull(flags = {}) {
  ...
  const reviewRoot = resolve(getReviewVaultRoot());
  const config = readCycleConfig(reviewRoot);
  const cycleStatusRoot = resolveCycleStatusRoot(config, reviewRoot);
...
function readCycleConfig(reviewRoot) {
  const primaryPath = resolveContainedChild(
    reviewRoot,
    join(...CONFIG_REL_PATH),
    reviewRoot,
    'market cycle config path'
  );
  const archiveRoot = resolve(getResearchVaultRoot());
  const archivePath = resolveContainedChild(
    archiveRoot,
    join(...CONFIG_REL_PATH),
    archiveRoot,
    'archived market cycle config path'
  );
  const configPath = existsSync(primaryPath) ? primaryPath : archivePath;
  if (!existsSync(configPath)) {
    throw new Error(`Missing market cycle config: ${primaryPath}`);
  }
...
function routeCycleStatusDir(configuredDir) {
  const normalized = normalizeDisplayPath(configuredDir || '').replace(/^\/+/, '');
  if (!normalized || normalized === '.') return 'Reports/Freshness/Market_Cycles';
  if (normalized.toLowerCase() === '01_freshness/market_cycles') {
    return 'Reports/Freshness/Market_Cycles';
  }
  return normalized;
}
...
function assertContainedPath(rootPath, targetPath, label) {
  ...
  if (rel !== '' && (rel.startsWith('..') || isAbsolute(rel))) {
    throw new Error(`${label} must stay under review vault root: ${target}`);
  }
  return target;
}
