...
await runTest('pull rejects cycle_status_dir that escapes the review vault root', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => pull({ dryRun: true }),
      /cycle_status_dir.*review vault root/i
    );
  } finally {
...
await runTest('pull dry-run plans only contained outputs and creates no missing files', async () => {
  ...
  try {
    process.env.REPORTS_VAULT_ROOT = tempRoot;
    const configPath = join(tempRoot, '99_System', 'config', 'market-cycle-monitor.config.json');
    const expectedOutput = join(tempRoot, 'Reports', 'Freshness', 'Market_Cycles', 'dry-run-probe.md');
    mkdirSync(dirname(configPath), { recursive: true });
    writeFileSync(
      configPath,
      JSON.stringify({
