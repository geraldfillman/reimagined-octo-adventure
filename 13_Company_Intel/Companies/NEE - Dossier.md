...
# NEXTERA ENERGY INC — Company Dossier
...
> Research angle (framework §14): Regulated returns, power demand, financing, generation mix
...
> Generate the filing inventory with `node run.mjs edgar baseline --ticker NEE` — link the latest pull note here.
...
> Generate from XBRL with `node run.mjs edgar facts --ticker NEE` — link the latest pull note here, then interpret.
