...
one_liner: "Owns warehouses near cities and ports and rents them to companies that move goods."
...
Owns warehouses near cities and ports and rents them to companies that move goods.

> Research angle (framework §14): Logistics-property rent, development, financing, tenant demand
...
> Generate the filing inventory with `node run.mjs edgar baseline --ticker PLD` — link the latest pull note here.
...
> Generate from XBRL with `node run.mjs edgar facts --ticker PLD` — link the latest pull note here, then interpret.
