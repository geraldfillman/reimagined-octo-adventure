...
# NOVO NORDISK A S â€” Company Dossier
...
> Generate the filing inventory with `node run.mjs edgar baseline --ticker NVO` â€” link the latest pull note here.
...
> Generate from XBRL with `node run.mjs edgar facts --ticker NVO` â€” link the latest pull note here, then interpret.
