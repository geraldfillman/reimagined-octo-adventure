...
> Generate the filing inventory with `node run.mjs edgar baseline --ticker WMT` â€” link the latest pull note here.
...
> Generate from XBRL with `node run.mjs edgar facts --ticker WMT` â€” link the latest pull note here, then interpret.
