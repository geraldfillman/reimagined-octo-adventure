...
one_liner: "Buys large quantities of goods, sells them at low markups, and charges customers for memberships."
...
# COSTCO WHOLESALE CORP /NEW — Company Dossier
...
Buys large quantities of goods, sells them at low markups, and charges customers for memberships.

> Research angle (framework §14): Membership economics, low-markup retail, renewal durability
...
> Generate the filing inventory with `node run.mjs edgar baseline --ticker COST` — link the latest pull note here.
...
> Generate from XBRL with `node run.mjs edgar facts --ticker COST` — link the latest pull note here, then interpret.
