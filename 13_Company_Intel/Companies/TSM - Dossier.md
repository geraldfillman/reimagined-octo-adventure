...
# TAIWAN SEMICONDUCTOR MANUFACTURING CO LTD â€” Company Dossier
...
> Generate the filing inventory with `node run.mjs edgar baseline --ticker TSM` â€” link the latest pull note here.
...
> Generate from XBRL with `node run.mjs edgar facts --ticker TSM` â€” link the latest pull note here, then interpret.
