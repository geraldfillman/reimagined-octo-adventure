...
one_liner: "Discovers, tests, manufactures, and sells medicines."
...
> Research angle (framework §14): Drug discovery, manufacturing capacity, patent and reimbursement risk
...
> Generate the filing inventory with `node run.mjs edgar baseline --ticker LLY` — link the latest pull note here.
...
> Generate from XBRL with `node run.mjs edgar facts --ticker LLY` — link the latest pull note here, then interpret.
